...
from capsule.layer import CapsuleLinear
...
class CapsuleTransformersCRF(nn.Module):
    def __init__(self, tag_vocabs, embed, num_layers, d_model, n_head, feedforward_dim, dropout,
                 after_norm=True, attn_type='adatrans', bi_embed=None,
                 fc_dropout=0.3, pos_embed=None, scale=False, dropout_attn=None):
        ...
        self.features = nn.GRU(self.embed.embed_size, self.hidden_size, num_layers=2, dropout=dropout, batch_first=True,
                               bidirectional=True)
        
        if classifier_type == 'capsule' and routing_type == 'k_means':
            self.classifier = CapsuleLinear(out_capsules=len(tag_vocabs[i]), in_length=self.in_length,
            ...
            self.classifier = nn.Linear(in_features=self.hidden_size, out_features=len(tag_vocabs[i]), bias=False)
