...
class RelativeSinusoidalPositionalEmbedding(RelativeEmbedding):
    ...
    def __init__(self, embedding_dim, padding_idx, init_size=1568):
        ...
        :param embedding_dim: dimension for each location
        :param padding_idx:
        :param init_size:
        """
        super().__init__()
...
class RelativeMultiHeadAttn(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        AC = torch.einsum('bnqd,bnkd->bnqk', [rw_head_q, k])  # b x n x l x d, n is head

        D_ = torch.einsum('nd,ld->nl', self.r_w_bias, pos_embed)[None, :, None]  # head x 2max_len, The bias of each head pair position
        B_ = torch.einsum('bnqd,ld->bnql', q, pos_embed)  # bsz x head  x max_len x 2max_len, the offset of each query to each shift
        E_ = torch.einsum('bnqd,ld->bnql', k, pos_embed)  # bsz x head x max_len x 2max_len, The bias of key to relative
        BD = B_ + D_  # bsz x head x max_len x 2max_len, to convert to bsz x head x max_len x max_len
